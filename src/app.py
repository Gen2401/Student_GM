...
@app.route('/nn_model/', methods=['post', 'get'])
def nn_model_page():
    ...
    # тестовый пример 42, var1=2.664389
    params = dict(zip(features.keys(), ['1996.159145', '525.057774', '77.506883', '18.126107',
        '223.408685', '28.658102', '69.489773', '2220.587445', '314.776669', '0', '6.67578', '78.623299']))
...
#            print(sample_x)
#            print(data)
            d2 = {'Соотношение матрица-наполнитель': 0}
            d2.update(data)
            data = d2
#            print(data)
            sample_x = pd.DataFrame(data, index=[0])
#            print(sample_x)
            sc = load_pickle_obj('scaler.pkl')
            sample_x_scaled = pd.DataFrame(sc.transform(sample_x),
                columns = sample_x.columns,
                index = sample_x.index)
#            print(sample_x_scaled)
            sample_x_pred = sample_x_scaled.drop('Соотношение матрица-наполнитель', axis=1)
            n_model = keras.models.load_model(MODELS_PATH + 'nn_model')
            pred_val = n_model.predict(sample_x_pred)
            print(pred_val)
            sample_x_scaled['Соотношение матрица-наполнитель'] = pred_val
            pred_y = pd.DataFrame(sc.inverse_transform(sample_x_scaled),
                columns = sample_x_scaled.columns,
                index = sample_x_scaled.index)
...
#            var1 = y3[0]
            var1 = pred_y['Соотношение матрица-наполнитель'].values[0]
    # Отображение результата
    return render_template('nn_model.html', params=params, error=error, inputs=sample_x.to_html(), var1=var1)
